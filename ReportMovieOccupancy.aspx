<%@ Page Title="Report - Movie Occupancy" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="ReportMovieOccupancy.aspx.cs" Inherits="CinemaTicketSystem.ReportMovieOccupancy" %>

<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
    <h2 class="page-title">Movie Occupancy Report</h2>

    <section class="card report-card">
        <h3>Top 3 Halls By Occupancy For Selected Movie</h3>
        <div class="form-grid report-filter-grid">
            <div class="field">
                <label for="ddlMovies">Movie</label>
                <asp:DropDownList ID="ddlMovies" runat="server" />
            </div>
        </div>
        <div class="actions">
            <asp:Button ID="btnLoad" runat="server" CssClass="btn btn-primary" Text="Load Report" OnClick="btnLoad_Click" />
        </div>
        <asp:Label ID="lblStatus" runat="server" CssClass="status" />
    </section>

    <section class="card">
        <asp:GridView ID="gvMovieOccupancy" runat="server" CssClass="gridview" AutoGenerateColumns="False">
            <Columns>
                <asp:BoundField DataField="Theater_Name" HeaderText="Theater" />
                <asp:BoundField DataField="Hall_Number" HeaderText="Hall" />
                <asp:BoundField DataField="Hall_Seating_Capacity" HeaderText="Capacity" />
                <asp:BoundField DataField="Sold_Seats" HeaderText="Sold Seats" />
                <asp:BoundField DataField="Occupancy_Percentage" HeaderText="Occupancy %" DataFormatString="{0:N2}" />
            </Columns>
        </asp:GridView>
    </section>
</asp:Content>
